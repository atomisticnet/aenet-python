...
provided by the ``aenet tools`` package, but rather shows a collection
...
*  Extract structures, energies, and atomic forces from VASP
   ...
   optimization.  The structure, total energy, and atomic forces will be
   written out to a file named ``final_structure.xsf`` in ænet's XSF
   format.
...
*  Extract all structures, their energies and atomic forces from VASP
   calculations.  This will generate a separate XSF file named
